...
function print_help {
  echo "Build the Docker image with the given tag and push the image if specified"
...
# Image
IMAGE_NAME="spark-delta"
IMAGE_FULL_TAG="$IMAGE_NAME:$IMAGE_TAG"

# Docker file
DOCKERFILE="Dockerfile"
echo "Using image from: $DOCKERFILE"

echo "Building $IMAGE_FULL_TAG"
docker build -f $DOCKERFILE -t "$IMAGE_FULL_TAG" .
