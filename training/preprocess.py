...
def preprocess(input_path: Path, output_dir: Path):
    ...
    df[['Month', 'Day', 'Hour', 'Year']] = df[['Month', 'Day', 'Hour', 'Year']].astype(int)
    ...
    test_len = train_valid_len + slice
    ...
    df_train.to_csv(output_dir / 'train.csv', index=None)
    df_valid.to_csv(output_dir / 'valid.csv', index=None)
